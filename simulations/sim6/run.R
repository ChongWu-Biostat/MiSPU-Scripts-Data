setwd("/panfs/roc/groups/1/panwei/wuxx0845/swak/sim6/")

#setwd("/Users/chong/Google Drive/SWAK/simulation/swak-bioinformatics-7-19/sim5/")
library(aSPU)
library("GUniFrac")
library("dirmult")
...
y <- scale(rowSums(OTU))[, 1] * 0 + rnorm(100 ) + 0.5*(X[,1] + X[,2])

out = swak(y,comm,cov=NULL,throat.tree,pow=c(0.5,1,1.5,2),out_type = "C")
...
y <- scale(rowSums(OTU))[, 1] * 0.25 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
...
y <- scale(rowSums(OTU))[, 1] * 0.5 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
...
y <- scale(rowSums(OTU))[, 1] * 0.75 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
...
y <- scale(rowSums(OTU))[, 1] * 1 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
...
y <- scale(rowSums(OTU))[, 1] * 1.25 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
...
y <- scale(rowSums(OTU))[, 1] * 1.5 + rnorm(100 ) + 0.5*(X[,1] + X[,2])
