...
# same as step4_withAdditonalCovariates.R, resulting paralele to step4.1.3
rm(list = ls())
...
outfile = sprintf("%s/pvalues_sz%d_fold%s_cluster12_cfd%d.txt", outdir, n ,as.character(f), a) 
...
for (set in 1:n.sim){
  ...
  y <- scale(rowSums(OTU))[, 1] * b * f + rnorm(n ) + 0.5*(X[,1] + X[,2])
  ...
  Du  = unifracs[,,"d_UW"]
  D0  = unifracs[,,"d_0"]
  D.25= unifracs[,,"d_0.25"]
  D.05= unifracs[,,"d_0.5"]
  D.75= unifracs[,,"d_0.75"]
  ...
  if (set == 1){
    header =  c("ps_w_zhou", "ps_u_zhou",  "ps_0_zhou","ps_25_zhou","ps_05_zhou","ps_75_zhou", "ps_bc_zhou","omni_ps_zhou", "minP_zhou",
                "ps_w_adj_zhou", "ps_u_adj_zhou",  "ps_0_adj_zhou","ps_25_zhou","ps_05_adj_zhou","ps_75_adj_zhou", "ps_bc_adj_zhou",
                "omni_ps_adj_zhou", "minP_adj_zhou")
