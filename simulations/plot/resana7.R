...
##
library(ggplot2)
res = read.table("sim7.txt",sep=",")

res[,3] = factor(rep(1:9,times = 7),levels =  1:9)
## create the plot
base_size = 12
base_family = ""
p1 = ggplot(res, aes(x=x, y=power,group = Methods, color = Methods,shape = Methods,linetype = Methods))+ annotate("text", x = 0.6, y = 0.66, label = "X,Z Correlated", size = 3.5) +
 annotate("text", x = 0.6, y = 0.625, label = "No Adjustment for X", size = 3.5) +
