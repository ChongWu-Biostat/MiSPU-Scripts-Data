...
##
library(ggplot2)
res = read.table("sim6.txt",sep=",")

res[,3] = factor(rep(1:9,times = 7),levels =  1:9)
## create the plot
base_size = 12
base_family = ""
p1 = ggplot(res, aes(x=x, y=power,group = Methods, color = Methods,shape = Methods,linetype = Methods))+ annotate("text", x = 1.2, y = 0.7, label = "X,Z Independent", size = 3.5) +
 annotate("text", x = 1.2, y = 0.66, label = "No Adjustment for X", size = 3.5) +
