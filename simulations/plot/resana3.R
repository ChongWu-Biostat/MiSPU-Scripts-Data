...
##
library(ggplot2)
res = read.table("sim3.txt",sep=",")

res[,3] = factor(rep(1:9,times = 6),levels =  1:9)
## create the plot
base_size = 12
base_family = ""
p1 = ggplot(res, aes(x=x, y=power,group = Methods, color = Methods,shape = Methods,linetype = Methods))+ annotate("text", x = 0.8, y = 0.29, label = "X,Z Correlated", size = 3.5) +
 annotate("text", x = 0.8, y = 0.275, label = "Adjustment for X", size = 3.5) +
