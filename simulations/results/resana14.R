...
setwd("/panfs/roc/groups/1/panwei/wuxx0845/swak/sim14/")
library(xtable)
plot.res = matrix(NA,9,6)
...
##
library(ggplot2)
res = as.data.frame(matrix(NA,9*6,3))
colnames(res) = c("power","x","Methods")
res[,1] = c(plot.res)
res[,2] = rep(c(0,0.6,0.8,1.2,1.6,2),each  = 9)
res[,3] = factor(rep(c("Ku","K5","Kw","Kopt","MiSPUw2","MiSPUwinf","aMiSPUw","aMiSPUu","aMiSPU"),times = 6),levels =  c("Ku","K5","Kw","Kopt","MiSPUw2","MiSPUwinf","aMiSPUw","aMiSPUu","aMiSPU"))

setwd("/panfs/roc/groups/1/panwei/wuxx0845/swak/results/")
write.table(res,"sim14.txt",sep=",")
